...
tr[, X_cols] <- lapply(tr[, X_cols], function(x) {
  if (is.character(x) || is.factor(x)) as.numeric(as.factor(x)) else x
})

va[, X_cols] <- lapply(va[, X_cols], function(x) {
  if (is.character(x) || is.factor(x)) as.numeric(as.factor(x)) else x
})

# small validation from tail of training (drives early stopping)
...
params <- list(
  objective        = "reg:squarederror", # (alt: "reg:squaredlogerror")
  eval_metric      = "rmse",
  max_depth        = 12,   # more capacity
  eta              = 0.03, # smaller LR
  subsample        = 1.0,
...
bst <- xgb.train(
  params = params,
  data = dtr,
  nrounds = 5000,                     # high cap; we rely on early stopping
  watchlist = list(train = dtr, valid = dva),
  early_stopping_rounds = 200,
  print_every_n = 50
)

# refit on full training using best_iteration
...
ggsave("actual_vs_predicted_hourly_log.png", p, width = 10, height = 4.5, dpi = 150)
print(p)
...
write_csv(total_metrics, "total_metrics_reconciled_simple.csv")
print(total_metrics)
